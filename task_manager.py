...
def _insert_task(
    ...
    conn.execute(
        """
        INSERT INTO tasks (
            task_id, device_id, asic_model, ambient_level, query_budget, target, created_at, expires_at, status
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open')
        """,
        (task_id, device_id, asic_model, ambient_level, query_budget, target, created_at, expires_at),
    )
...
def save_task_bundle_to_file(bundle: TaskBundle, output_path: Path | str) -> None:
    """
    Save a generated TaskBundle to a JSON file.
    """
...
def main() -> None:
    """
    CLI entry point for generating a default task bundle.

    - ASIC model: Antminer S19
    - Devices: 5
    - Ambient levels: all 5 defined levels
    - Output: tasks.json in current directory
    """
    bundle = generate_miner_task_bundle()
    save_task_bundle_to_file(bundle, Path("tasks.json"))
    print("Generated task bundle and saved to tasks.json")
